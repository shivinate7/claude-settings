...
13. The orchestrator and product code: **judge the act, not the file.** This session writes records, docs, and briefs. Building goes to a lane. A fix under ten lines with no test change is the exception, named in the report. Reason, measured here: "It never edits product code" read as a file test. It blocked a decision entry, which is orchestration, and charged a whole lane for one comment.
