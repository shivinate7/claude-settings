...
| --- | --- | --- | --- |
| Shared trees: `git stash push`/`save`/bare, `stash drop`, `stash clear`, `git reset --hard`, `git restore` with neither flag or with `--worktree`, `git clean -f`, `git checkout <path>` | `Bash`, `PowerShell` | deny in a shared checkout, ask in a git worktree | mutation-test with a `.bak` copy, or work in a worktree of your own |
...
13. The orchestrator and product code: **judge the act, not the file.** This session writes records, docs, and briefs itself. A change to behavior goes to a lane, however small it looks. Two exceptions carry a report line: a fix faster to make than to brief, and a code change of ten lines or less that needs no test change. Reason, measured in this session: the old sentence, "It never edits product code", read as a file test. It blocked a decision entry, which is orchestration, and it charged a whole lane for one comment. The owner ruled the comment fix acceptable and asked for the line to move. The budget is a guide for the reader, and the act still decides.
